...
Ext.extend(Ext.ux.grid.GridSummary, Ext.util.Observable, {
  init : function(grid) {
    ...
    grid.on('columnresize',this.doWidth, this);
  ...
  doAllWidths: function(ws, tw){
	var gridColumn = this.grid.getColumnModel().config;
	var columns =[];
    for(var i = 0 ; i<gridColumn.length; i++){
		var colWidth = gridColumn[i].width;
		columns.push(colWidth-2);
	}
	var s = this.view.summary.dom;
    var cells = s.firstChild.rows[0].childNodes;
	for(var i = 0; i<cells.length; i++){
		var colWidth = columns[i]+"px";
		s.firstChild.rows[0].childNodes[i].style.width = colWidth;
	}
	if(this.jsonVbak!=null){
		this.setSumValue(this.jsonVbak);
	}
  },
  ...
  setSumValue : function(jsonV) {
    ...
    for (var i = 0, len = cs.length; i < len; i++) {
      ...
      if (jsonV&&jsonV[c.name]&&i!=1) {
        if(jsonV[c.name] != '合计'){
 	       var r = /^\d*\.\d*$/;
	       var per = /%$/; //处理百分号问题
           var v = jsonV[c.name].trim();
	   	   if(per.test(v)){
	   		  v = v.substring(0,v.length-1);
	   		  //v = Number(jsonV[c.name]);
	   		  if(!r.test(v)){
