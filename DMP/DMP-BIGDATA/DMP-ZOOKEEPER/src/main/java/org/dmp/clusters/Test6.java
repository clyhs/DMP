...
// 	   zkClient.createPersistent("/uc/cluster");
		...
		if(unit==-8){
			List<Config<String>> list = new ArrayList<Config<String>>();
			Map<String, byte[]> map = null;
				map = cm.getConfigs(0, 0);
				Config<String> config = null;
				Set<String> keys = map.keySet();
				for(String key : keys){
					System.out.println("key:"+key+"   value:"+new String(map.get(key)));
					config = new Config<String>(key, new String(map.get(key)));
					list.add(config);
				}
				return;
 		}
...
//		} 
		...
		if (unit == 0) {
			...
			server.setServerIp("172.17.151.22");
			server.setServerName("mup-401");
			int rst = cm.addOrUpdateServer(server);
			System.out.println("add server,ServerName:mup-401:" + rst);
...
//		        cm.addOrUpdateConfig(i, 0, BaseConfigKey.MSAGRES_SERVICE_PATH, "string:http://192.168.1.48:8080/res".getBytes());
				...
				cm.addOrUpdateConfig(i, 1, BaseConfigKey.MGP_SERVER_IP, "string:10.1.2.7:8899".getBytes());
				cm.addOrUpdateConfig(i, 1, BaseConfigKey.MGP_SERVER_PORT, "int:8899".getBytes());
