 ...
 */
package org.dmp.core.db.hbase.ref;
...
/**
 * Central class for accessing the HBase API. Simplifies the use of HBase and
 * helps to avoid common errors. It executes core HBase workflow, leaving
 * application code to invoke actions and extract results.
 * 
 * @author Costin Leau
 */
public class HbaseTemplate extends HbaseAccessor implements HbaseOperations
{
	...
	{
		return HbaseUtils.getHTable(tableName, getConfiguration(),
				getCharset(), getTableFactory());
	}
	...
	private void flushIfNecessary(HTableInterface table, boolean oldFlush)
			throws IOException
	{
	...
	public <T> T find(String tableName, String family,
	...
	public <T> T find(String tableName, final Scan scan,
			final ResultsExtractor<T> action, final int nStart, final int nOffset)
	{
		return execute(tableName, new TableCallback<T>()
		{
			public T doInTable(HTableInterface htable) throws Throwable
			{
				...
				try
				{
					return action.extractData(scanner, nStart, nOffset);
				} finally
				{
					scanner.close();
				}
	...
	public <T> List<T> find(String tableName, final Scan scan,
			final RowMapper<T> action, final int nStart, final int nOffset)
	{
		return find(tableName, scan, new RowMapperResultsExtractor<T>(action), nStart, nOffset);
	}
