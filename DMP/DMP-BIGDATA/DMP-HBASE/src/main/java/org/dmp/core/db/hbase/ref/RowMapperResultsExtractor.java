...
class RowMapperResultsExtractor<T> implements ResultsExtractor<List<T>> {
	...
	public List<T> extractData(ResultScanner results, int nStart, int nOffset) throws Exception {
		List<T> rs = new ArrayList<T>();
		int rowNum = -1;
		int nTotal = nStart + nOffset;
		for (Result result : results) {
			rowNum++;
			if(rowNum < nStart) continue;
			if(rowNum >= nTotal) break;
			rs.add(this.rowMapper.mapRow(result, rowNum));
		}
		return rs;
	}
